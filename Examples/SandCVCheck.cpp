...
//    cout<<"size activeList = "<<activeList.size()<<endl;
    ...
    //--------------------------------------------------------------------------
    //__________________________________________________________________________
    // [2-0] Initialize SandCV
    //
    SandCV sandcv(sdim, ldim, hdim, spacing, xRef, activeList, xiNodes, xNodes, xiSeeds);
//    xSamples = sandcv.GetHighSeeds();
//    cout<<"size xSample ="<<xSamples.size()<<endl;
    //__________________________________________________________________________
    // [2-1] Check the Jacobian of SandCV with finite differences
    //
    int iD, jD;
    int iStart = 0;
    ...
    vector<double> jacSandCV(nTests*hdim*ldim, 0.0);
...
//            cout<<iter<<" ";
//        cout<<"]"<<endl;
        sandcv.CalculateCVdist( xOrig );
        copy( sandcv.GetJacobian().begin(), sandcv.GetJacobian().end(), jacSandCV.begin()+iN*hdim*ldim  );
        copy( sandcv.GetValue().begin(), sandcv.GetValue().end(), xiProjected.begin()+iN*ldim  );

        //        cout<<"dihedral [ "<<dihedrals.at((iStart+iN)*2)<<" , "<<dihedrals.at((iStart+iN)*2+1)<<"]"<<endl;
//        cout<<"cv_value [ "<<sandcv.GetValue().at(0)<<" , "<<sandcv.GetValue().at(1)<<"]"<<endl;
//        cout<<"________________________________________"<<endl;
//        for( iD = 0; iD < hdim; ++iD )
//        {
//            copy( xOrig.begin(), xOrig.end(), xTemp.begin() );
//            xTemp.at(iD) += deltaX;
//            sandcv.CalculateCVdist( xTemp );
//            xiPlus = sandcv.GetValue();
////            cout<<"+cv = [ "<<xiPlus.at(0)<<" , "<<xiPlus.at(1)<<"]"<<endl;

//            copy( xOrig.begin(), xOrig.end(), xTemp.begin() );
//            xTemp.at(iD) -= deltaX;
//            sandcv.CalculateCVdist( xTemp );
//            xiMinus = sandcv.GetValue();
...
//    cout<<"jacs ="<<endl;
//    for ( iD = 0; iD < hdim; ++iD)
//        cout<<"("<<jacSandCV.at(iD)<<","<<jacFinite.at(iD)<<") ";
//    cout<<endl;
...
//    cout<<endl;
    ...
    IO.WritePointSet(prefix+"jacobian_sandcv.txt", jacSandCV  , nTests, hdim*ldim, 8);
